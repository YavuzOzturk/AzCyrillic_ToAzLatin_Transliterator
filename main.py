...
def read_xlsx(fileName):
    ...
    if (fileName[-1:] == 'x'):
        ...
        wb_obj_xls = xlrd.open_workbook(excel_file)
        sheet = wb_obj_xls.sheet_by_index(0)
        # wb = Workbook()
        # ws = wb.active
        # ws.title = "Decoded_Xlsx"
        height = 1
        ...
        for r in range(0, sheet.nrows):
            ...
            for c in range(0, sheet.ncols):
                ...
                row_curr += temp + "\",\""
                cell += 1
            write_csv(fileName.replace(".xls", ".csv"), (row_curr[:-2] + "\n"))
        height += 1
    print(fileName)
